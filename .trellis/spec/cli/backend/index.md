...
| [Release Process](./release-process.md) | CI-only publishing, package versioning, release tracks, manifest continuity | Done |
| [Core Module Boundary](./trellis-core-sdk.md) | `src/core/` ↔ CLI boundary, public entry points, build and versioning contracts | Done |
| [Platform Integration](./platform-integration.md) | Upstream multi-platform integration patterns (historical reference; Kerminal is the only platform now) | Reference |
...
- Modifying `init.ts` flow (new triggers, dispatch branches, bootstrap/joiner) → [platform-integration.md "Bootstrap & Joiner Task Auto-Generation"](./platform-integration.md) — two-point wiring + `.developer` signal (historical reference; adapt to the Kerminal-only registry)
...
- Editing `packages/cli/src/core/**`, moving reusable CLI logic into core, or changing core imports → [trellis-core-sdk.md](./trellis-core-sdk.md)
