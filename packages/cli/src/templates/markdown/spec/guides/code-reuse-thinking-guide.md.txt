...
**Problem**: Python's if/elif/else chains have no compile-time exhaustive check. When you add a new value to a `Literal` type (e.g., `ProjectType`), existing if/elif/else chains silently fall through to `else` with wrong defaults.

**Symptom**: The new value works partially — some methods keep returning defaults for a different value instead of handling the new one. No error is raised.

**Example**:
```python
# BAD: "fullstack" falls through to else, returns "frontend"
@property
def default_layer(self) -> str:
    if self.project_type == "backend":
        return "backend"
    else:
        return "frontend"  # fullstack silently gets "frontend"!

# GOOD: explicit branch for every project type
@property
def default_layer(self) -> str:
    if self.project_type == "backend":
        return "backend"
...
**Real example**: `trellis update` had a manual `files.set()` list for 11 scripts that `getAllScripts()` already tracked. Fix: replaced the manual list with a `for..of getAllScripts()` loop. See the earlier `update.ts` refactor.
