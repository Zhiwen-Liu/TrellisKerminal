...
| --------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--platform claude\|codex\|devin\|grok\|kerminal\|opencode\|pi\|zcode\|all` | all               | Default `all`. Eight data sources: kerminal, claude, codex, devin, grok, opencode, pi, zcode. `devin` is Cognition Devin CLI (`sessions.db`). |
...
- **OpenCode sessions come from a SQLite store** (`~/.local/share/opencode/opencode.db`) read through a zero-dependency SQLite reader. Phase slicing is not supported for OpenCode sessions — `--phase` on platform `opencode` returns the full dialogue with a warning; use `--grep` or `--phase all` instead.
- **`--platform devin` is Cognition Devin CLI** (`~/.local/share/devin/cli/sessions.db`). It is not Devin Desktop / Cascade and not Factory Droid.
...
- **`mem` reads local session stores directly** — JSONL for Kerminal / Claude / Codex / Grok / Pi, SQLite for Devin / OpenCode / ZCode. If the user has cleared their session storage, `mem` cannot recover what is no longer on disk.
