...
Kerminal has no project-level sub-agent registry, so the main session dispatches a generic sub-agent:

1. Load the `trellis-research` skill (`.kerminal/skills/trellis-research/SKILL.md`).
2. Spawn a generic sub-agent whose initial message is the skill content, prefixed with:
   - `Active task: <path from task.py current>` as the first line
   - A statement that the spawned agent is already `trellis-research` and may write only under the active task's `research/` directory
